...
if [ "$(ls -A "$TV_CONVERT")" ]; then
 rm "$TV_CONVERT/*-converted.mp4" > /dev/null 2>&1
 FILE=$(ls -tr "$TV_CONVERT" | tail -1)
 { time $ENCODE_SCRIPT "$TV_CONVERT/$FILE" "TV"; } 2> /tmp/time_tv.txt
 mv "$TV_CONVERT/${FILE%\.*}-converted.mp4" "${TV_IMPORT}/" && rm -f "$TV_CONVERT/${FILE}" && chmod 777 "${TV_IMPORT}/${FILE%\.*}-converted.mp4" && echo "[TV Show] ${FILE} [$(date "+%a %D %H:%M")] `cat /tmp/time_tv.txt | grep sys | awk '{print $2}'`" >> "${LOG}" || touch -d "2000-01-01 00:00:00" "$TV_CONVERT/${FILE}"
fi
rm "$PIDFILE"
